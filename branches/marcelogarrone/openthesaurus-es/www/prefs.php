...
if( array_key_exists('change_settings', $_POST) ) {
	if( $_POST['change_settings'] == 1 && $_POST['pw1'] != $_POST['pw2'] ) {
		$msg = T_("<p class=\"error\">Error: passwords don't match.</p>");
	} elseif( $_POST['change_settings'] == 1 && trim($_POST['pw1']) != "" &&
			strlen($_POST['pw1']) <= 3 ) {
		$msg = T_("<p class=\"error\">Error: password is too short (minimum length is 4 characters).</p>");
		...
		if( trim($_POST['pw1']) != "" ) {
			...
			$db->query($query);
			$msg .= T_("<p class=\"okay\">New password has been set and will become active for the next login.</p>");
		}
		if( trim($_POST['visiblename']) != $_POST['visiblename_old'] ) {
			...
			$db->query($query);
			$msg .= T_("<p class=\"okay\">New visible name has been set.</p>");
		}
		if( $msg == "" ) {
			$msg .= T_("<p class=\"error\">No changes made.</p>");
		}		
	}
}
...
<table border="0">
<tr>
	<td valign="top"><?php print T_("Login:") ?></td>
	<td valign="top"><span class="naviusername"><?php print escape($db->f("user_id")) ?></span></td>
</tr>
<tr>
	<td valign="top"><?php print T_("New password:") ?></td>
	<td valign="top"><input type="password" name="pw1" /></td>
	<td valign="top"><?php print T_("Leave empty to keep your current password.") ?></td>
</tr>
<tr>
	<td valign="top"><?php print T_("New password (again):") ?></td>
	<td valign="top"><input type="password" name="pw2" /></td>
</tr>
<tr>
	<td valign="top"><?php print T_("Visible name:") ?></td>
	...
	<td valign="top"><?php print T_("This name will appear in the statistics that are publicly visible. Leave empty if you want to stay anonymous.") ?></td>
</tr>
<tr>
	<td></td>
	<td><?php print "<input type=\"submit\" value=\"" . T_("Change") . "\" />" ?></td>
</tr>
</table>
</form>


<h2><?php print T_("Personal Statistics") ?></h2>

<table border="0">
<tr>
	<td><?php print T_("Words and synsets added:") ?></td>
	<td>
		<?php
...
<tr>
	<td><?php print T_("Words and synsets removed:") ?></td>
	<td>
		<?php
		$query = sprintf("SELECT count(*) AS ct FROM user_actions_log
			WHERE 
