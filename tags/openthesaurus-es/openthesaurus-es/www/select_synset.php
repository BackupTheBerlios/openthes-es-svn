...
<p><?php print(sprintf(T_("Please select a superordinate concept for <strong>%s</strong>:"),
	join(', ', getSynset($_GET['meaning_id'], 3)))) ?></p>

<?php
$syns = getSynset($_GET['meaning_id']);
if( array_search($word, $syns) ) {
	print sprintf(T_("Error: <strong>%s</strong> cannot be a superordinate concept, as it already appears in the current synset. Please go back and try again."),
		escape($word));
} else {
	?>

	<label class="myhoverbright"><input type="radio" name="super_id" value="nothingselected" checked="checked" /><?php print T_("Please select below:"); ?></label><br />
	<label class="myhoverbright"><input type="radio" name="super_id" value="create" /><?php print sprintf(T_("Create a new synset containing '%s' as the first word"), escape($word)); ?></label><br />
	...
	while( $db->next_record() ) {
		...
		?>
		<span class="myhoverbright"><label><input type="radio" name="super_id" value="<?php print $db->f('meaning_id')?>" /><?php print T_("Use synset:") ?></label>
	...
	<br />
	<?php print '<input type="submit" value="'.T_("Continue").'" />'; ?>
<?php } ?>
